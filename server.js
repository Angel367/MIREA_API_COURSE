...
server.addService(phonebookProto.SubscriberService.service, {
    ...
    update: (call, callback) => {
        ...
        if (existingSubscriber) {
            existingSubscriber.name = call.request.name;
            existingSubscriber.surname = call.request.surname;
            existingSubscriber.number = call.request.number;
            existingSubscriber.age = call.request.age;
            existingSubscriber.address = call.request.address;
            callback(null, existingSubscriber);
        } else {
            callback({
                code: grpc.status.NOT_FOUND,
                details: "Не найдено"
    ...
    remove: (call, callback) => {
        let existingSubscriberIndex = subscribers.findIndex(
            n => n.id == call.request.id
        );
        if (existingSubscriberIndex != -1) {
            subscribers.splice(existingSubscriberIndex, 1);
            callback(null, {});
        } else {
            callback({
                code: grpc.status.NOT_FOUND,
